...
void dsv_i2c_configure(int mode)
{
	if (mode == DW_SET_ENM_R) {
		// DSV EN Signal from I2C in DW8768
		p1_dsv_write_byte(p1_dsv_base, 0x05, 0x0F);
	} else if (mode == DW_HIZ_KNOCK_ON) {
		// DSV HIZ set in DW8768
		p1_dsv_write_byte(p1_dsv_base, 0x03, 0x80);
		mdelay(1);
	} else if (mode == DW_HIZ_KNOCK_ON_ENM_R) {
		// DSV HIZ, KNOCK_ON and EN Signal set in DW8768
		mdelay(1);
		p1_dsv_write_byte(p1_dsv_base, 0x03, 0x80);
		mdelay(1);
		p1_dsv_write_byte(p1_dsv_base, 0x07, 0x08);
		mdelay(1);
	}
}
EXPORT_SYMBOL_GPL(dsv_i2c_configure);

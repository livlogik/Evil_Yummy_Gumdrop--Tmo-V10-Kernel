...
//	struct delayed_work *dwork = to_delayed_work(work);
	...
	if (chip->wlc_chg_en && check_counter >= 2) {
		...
		pr_info("[WLC] chg_en : %s, chg_mode : %s, chg_done : %s\n",
				wpc_mode ? "wpc_mode" : "pma_mode",
				wlc_status ? "enable" : "disable",
				chg_done ? "Done" : "Not yet");
		pr_info("[WLC] Op_freq : %d MHz, Limit_cur : %d mA, Out_cur %dmA\n",
				op_freq, limit_cur_ma, out_cur_ma);
		pr_info("[WLC] target_vol : %dmV, Out_vol : %dmV, Vrect : %dmV\n",
				target_vol, out_vol_mv, rect_vol_mv);
		pr_info("[WLC] Die_temperature : %d, Axis(%s%d, %s%d)\n", die_temp,
				temp_sign_x ? "-" : "+", chip->x_axis,
				temp_sign_y ? "-" : "+", chip->y_axis);
		pr_info("[WLCS] Fod1_gain : %d.%d%%, Fod2_gain : %d.%dmW\n",

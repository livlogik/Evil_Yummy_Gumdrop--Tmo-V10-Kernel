...
extern unsigned long rtc_fake_secs;
unsigned long misc_pwron_secs;

#define FTM_BLOCK_SIZE		2048
#define FTM_RTC_OFFSET		200
#define FTM_RTC_WRITE_SIZE	8
...
void write_rtc_pwron_in_misc(struct rtc_wkalrm *alarm)
{
	...
	if (fd < 0) {
		...
		pr_debug("[%s %d] org alarm time = %lu, fake rtc = %lu, fin time =%lu\n",
			__func__, __LINE__, rtc_time, rtc_fake_secs, rtc_time-rtc_fake_secs);

		alarm_info.time = rtc_time-rtc_fake_secs;
		misc_pwron_secs = alarm_info.time;

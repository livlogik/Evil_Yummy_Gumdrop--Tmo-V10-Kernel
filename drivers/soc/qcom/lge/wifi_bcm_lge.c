...
#if defined (CONFIG_BCM43455)
const struct cntry_locales_custom bcm_wifi_translate_custom_table_bcm43455[] = {
...
static void *get_country_code(char *ccode, const struct cntry_locales_custom *cc_table, int size)
{
	int i;
	...
	for (i = 0; i < size; i++) {
		if (strcmp(ccode, cc_table[i].iso_abbrev) == 0) {
			return (void *)&cc_table[i];
		}
	}
...
static void *bcm_wifi_get_country_code(char *ccode)
{
	...
	/*WLAN Modified country code from base country code table*/
#if defined (CONFIG_BCM43455)

	size = ARRAY_SIZE(bcm_wifi_translate_custom_table_bcm43455);
	result = (void*)get_country_code(ccode, bcm_wifi_translate_custom_table_bcm43455, size);
	if(NULL != result) {
		return (void*)result;
	}
#endif

	/*WLAN Normal country codes from BCM4339*/
	size = ARRAY_SIZE(bcm_wifi_translate_custom_table);
	result = (void*)get_country_code(ccode, bcm_wifi_translate_custom_table, size);
	if(NULL != result) {
		return (void*)result;
	}

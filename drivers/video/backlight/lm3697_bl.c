...
static void lm3697_set_main_current_level(struct i2c_client *client, int level)
{
	...
	{
		pr_debug("%s lm3697_write_reg... off??? level : %d\n", __func__, level);
		cal_value = level;
...
void lm3697_backlight_on(int level)
{
	if (backlight_status == BL_OFF) {
		...
		pr_debug("[backlight] %s Enter lm3697 initial\n", __func__);
		...
		lm3697_write_reg(main_lm3697_dev->client, 0x19, 0x00); //Sink Feedback Disable
		//lm3697_write_reg(main_lm3697_dev->client, 0x18, 0x13);	//Bank B Full-scale current (20.2mA)
		lm3697_write_reg(main_lm3697_dev->client, 0x10, 0x04);	//HVLED 1, 2 bankA, HVLED 3 bank B enable

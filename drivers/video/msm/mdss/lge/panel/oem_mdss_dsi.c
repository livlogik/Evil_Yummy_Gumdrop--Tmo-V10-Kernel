					...
					unsigned long event, void *data)
{
	...
	if(ctrl_pdata->lge_pan_data->always_on_power_ctrl) {
		if(event != LCD_EVENT_TOUCH_DRIVER_REGISTERED)
			return 0;
	}
#endif
	switch (event) {
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_SUPPORT_MFTS)
			if(ctrl_pdata->lge_pan_data->always_on_power_ctrl)
					lge_mdss_dsi_panel_power_ctrl_pre(&ctrl_pdata->panel_data, 1);
#endif
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_SUPPORT_MFTS)
			if(ctrl_pdata->lge_pan_data->always_on_power_ctrl)
				lge_mdss_dsi_panel_power_ctrl_post(&ctrl_pdata->panel_data, 1);
#endif
		}
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_SUPPORT_MFTS)
			if(ctrl_pdata->lge_pan_data->always_on_power_ctrl)
				lge_mdss_dsi_panel_power_ctrl_pre(&ctrl_pdata->panel_data, 0);
#endif
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_SUPPORT_MFTS)
			if(ctrl_pdata->lge_pan_data->always_on_power_ctrl)
				lge_mdss_dsi_panel_power_ctrl_post(&ctrl_pdata->panel_data, 0);
#endif
		}
	}
	return ret;

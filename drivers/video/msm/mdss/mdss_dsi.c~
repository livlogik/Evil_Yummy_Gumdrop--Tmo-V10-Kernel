...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	 ...
	 */
#if IS_ENABLED(CONFIG_LGE_DISPLAY_TUNING)
	if (!pdata->panel_info.ulps_suspend_enabled || !ctrl_pdata->ulps || refresh_clk_rate) {
#else
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_EXTENDED_PANEL)
		/* Do nothing for PPLUS LGD when suspend-ulps is enabled
		for decrease of current consumption. But for MIPI spec,
		DSI power must be enabled to keep MIPI lanes low.*/
#else

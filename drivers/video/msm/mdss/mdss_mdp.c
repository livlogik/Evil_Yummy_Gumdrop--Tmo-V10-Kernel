...
#ifdef CONFIG_MACH_MSM8992_PPLUS_CMCC_CN
#include <linux/spmi.h>
static int mdp_flag;
module_param_named(mdp_set, mdp_flag, int, S_IRUSR | S_IWUSR);
...
static void mdss_mdp_hw_rev_caps_init(struct mdss_data_type *mdata)
{
	...
	case MDSS_MDP_HW_REV_110:
		...
		mdata->max_target_zorder = 5; /* including base layer */
		mdata->max_cursor_size = 128;
...
#ifdef CONFIG_MACH_MSM8992_PPLUS_CMCC_CN
	ret =  mdss_mdp_suspend_sub(mdata);
	if(mdp_flag) {
		ctrl = spmi_busnum_to_ctrl(0);
		spmi_ext_register_writel(ctrl, 0x1, 0x4146, &reg, 1); // ldo2
		spmi_ext_register_writel(ctrl, 0x1, 0x5b46, &reg, 1); // ldo28
	}
